...
export function MenuItemEditForm({ item, categories, onSuccess }: MenuItemEditFormProps) {
  ...
  const form = useForm<MenuItemFormValues>({
    ...
    defaultValues: {
        ...
        imageType: item.image.startsWith('http') ? 'link' : 'file',
  ...
  async function onSubmit(data: MenuItemFormValues) {
    setIsLoading(true);
    try {
        let imageToUpdate: File | string | undefined = undefined;

        if (data.imageType === 'file' && data.imageFile && data.imageFile.length > 0) {
            imageToUpdate = data.imageFile[0];
        } else if (data.imageType === 'link' && data.imageUrl && data.imageUrl !== item.image) {
            imageToUpdate = data.imageUrl;
        }

        const { imageFile, imageUrl, imageType, ...itemData } = data;

        await updateMenuItem(item.id, itemData, imageToUpdate);
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormItem className="space-y-3">
                    ...
                    <FormControl>
                        ...
                        <FormItem className="flex items-center space-x-2 space-y-0">
                            ...
                            <FormLabel className="font-normal">Upload New Image</FormLabel>
                        </FormItem>
                        <FormItem className="flex items-center space-x-2 space-y-0">
                            <FormControl>
        ...
        {imageType === 'file' ? (
            <FormField
                ...
                render={() => (
                    <FormItem>
                        <FormLabel>New Food Image (Optional)</FormLabel>
                        <FormControl>
                            <Input 
                                type="file" 
